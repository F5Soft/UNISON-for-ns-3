      ...
      for (auto &p : flowMonitor->GetFlowStats ())
        {
          ...
          totalThroughput += (double) p.second.txBytes / (p.second.timeLastRxPacket - p.second.timeFirstTxPacket).GetSeconds ();
        }
      double avgFct = (double) totalFct.GetMicroSeconds () / flowMonitor->GetFlowStats ().size ();
      double avgDelay = (double) totalDelay.GetMicroSeconds () / totalRx;
      double avgThroughput = totalThroughput / flowMonitor->GetFlowStats ().size () / 1e9 * 8;
      LOG ("  Average flow completion time = " << avgFct << "us");
